...
export default function RightData() {
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem("rightWindowPath", pathDir);
  }, [pathDir]);
  ...
  return (
    <div className="max-w-[45%] w-full px-5 py-4">
      ...
      <div
        className="rounded border-2 border-[#f0f0f0] overflow-scroll scrollbar-hide"
        style={{ height: fileWindowHeight }}
        onClick={() => {
          const itemContextMenus = document.querySelectorAll(
            ".item-context-menus"
          );
          itemContextMenus.forEach((contextMenu) => {
